...
class Reactions:
    ''' Class defining the reactions features. Here reactions can involve
    ...
    def __init__(self, atom_features, reaction_features, num_of_types, \
        ...
        self.atom_features_in_reactions = {}
    ...
    def update_reactions_and_real_bonds(self, bond_change_frame, real_bonds, frame):
        # Check the reactions that happened during the frame and get their feature, if they happened before just
        # add an occurrence, otherwise add the reaction feature to the list of reaction, their type and count an
        # occurrence.
        ...
        # For each elementary feature, get its feature, save it, if 
        # needed and update the different variables.
        for reax in range(len(elementary_reactions_atoms)):
            reax_feature = self.get_reaction_feature(reax, \
            ...
            # one.
            if reax_feature.shape[0] == 0:
                continue

            found = False
            for i in range(len(self.reaction_dict)):
                if np.array_equal(reax_feature, self.reaction_dict[i]):
                    self.reaction_occurrences[i] += 1
                    self.reaction_occurrences_per_frame[i].append(frame)
                    found = True
                    break
            if not found:
                self.reaction_dict[len(self.reaction_dict)] = reax_feature
                self.atom_features_in_reactions[ \
                  len(self.reaction_dict) - 1] \
                    = np.unique( \
                        self.get_atom_features_in_reaction( \
                            reax_feature), axis=0)
...
#            real_bonds[bond_change_frame[i, 0], bond_change_frame[i, 1]] = bond_change_frame[i, 2]
    ...
    def get_reaction_feature(self, reax, elementary_reactions_atoms, \
                                 bond_changes_in_reactions, real_bonds):
        # Get the feature of the reaction depending on the features you are
        # interested in.
        ...
        # Get the feature of each atom in the reaction.
        atoms_features = np.zeros([num_atoms_reax, \
                                       self.num_atom_features], dtype=int)
        atom_idx = {}
        for num_atom, atom in enumerate(atoms_in_reaction):
            atoms_features[num_atom] \
                            = self.atoms.get_atom_features(atom, real_bonds)
            atom_idx[atom] = num_atom

        # Get the order of the atoms features, by sorting with the first
        # column, then by the second, ...
        order = np.lexsort( \
         atoms_features[:, range(self.num_atom_features - 1, -1, -1)].T)
        feature, feature_inverse, feature_counts \
                        = np.unique(atoms_features, return_inverse=True, \
                                    return_counts=True, axis=0)

        # If two or more atoms have the same feature, there are several
        # possibilities for the reaction feature, this case is treated
        # in different_features_possible_case(). The other case is treated
        # in only_one_feature_case().
        if (feature_counts > 1).any() and num_atoms_reax > 2:
            reax_feature \
                = self.different_features_possible_case( \
                        feature, feature_inverse, atoms_features, \
                        atom_idx, num_atoms_reax, num_bond_changes_reax,\
                        bond_changes_in_reaction, order)
        else:
            reax_feature \
                = self.only_one_feature_case(num_atoms_reax, \
    ...
    def different_features_possible_case(self, feature, \
                                             feature_inverse, \
                                             atoms_features, atom_idx, \
                                             num_atoms_reax, \
                                             num_bond_changes_reax, \
                                             bond_changes_in_reaction, order):
        # Obtain the reaction feature if some atoms have the same
        # features by running all the possible combinations of the reaction
        # feature and keeping the smallest in lexicographic order.

        # Get all the possible features
        array_poss = []
        for i in range(feature.shape[0]):
            atoms_with_feature = np.where(feature_inverse == i)[0]
            atoms_with_feature_order \
              = np.where((order[:, None] == atoms_with_feature).any(-1))[0]
            possible_orders \
                        = itertools.permutations(atoms_with_feature_order)
            array_poss.append(list(possible_orders))
        possibilities = list(itertools.product(*array_poss))

        # Get the reaction features of all the possible configurations.
        size_reaction_feature = 1 \
                                    + num_atoms_reax*self.num_atom_features \
                                    + 3*num_bond_changes_reax
        reax_feature_possibilities \
                = np.zeros([len(possibilities), size_reaction_feature], \
                           dtype=int)

        for i in range(len(possibilities)):
            order_temp = order[np.concatenate(possibilities[i])]
            reax_feature_possibilities[i, :] \
                = self.only_one_feature_case(num_atoms_reax, \
                                                 num_bond_changes_reax, \
                                                 atoms_features, \
                                                 bond_changes_in_reaction, \
        ...
        final_order = np.lexsort( \
                        reax_feature_possibilities[:, \
                        range(size_reaction_feature - 1, -1, -1) \
                        ].T)
        return reax_feature_possibilities[final_order[0], :]

    def only_one_feature_case(self, num_atoms_reax, num_bond_changes_reax, \
                                  atoms_features, bond_changes_in_reaction,\
                                  atom_idx, order):
        # Obtain the reaction feature when there is only one feature
        # possible.
        size_reaction_feature = 1 \
                                    + num_atoms_reax*self.num_atom_features \
                                    + 3*num_bond_changes_reax
        reax_feature = np.zeros([size_reaction_feature], dtype=int)

        # First a reaction is described by the number of atoms involved in the
        # reaction.
        reax_feature[0] = num_atoms_reax

        # Then it is described by the ordered atom features.
        reax_feature[1:1 + num_atoms_reax * self.num_atom_features] \
                    = atoms_features[order, :].reshape(-1)
        ...
        reax_feature[1 + num_atoms_reax * self.num_atom_features:] \
                                        = bond_changes_in_reaction_with_idx \
                                            .reshape(-1)
        return reax_feature

    def get_atom_features_in_reaction(self, reax_feature):
        # Get the list of the atom features for this reaction. 
        features = np.zeros([reax_feature[0], \
                                 self.num_atom_features], dtype=int)
        for i in range(reax_feature[0]):
            features[i] = reax_feature[1 \
                                    + self.num_atom_features*i: 1 \
                                    + self.num_atom_features*(i+1)]
        return features

    def update_clusters_to_follow_and_clusters_in_reactions(self, reax_feature):
        # Get the atom features of the atoms in the reaction.
        atom_features = self.get_atom_features_in_reaction( \
                                                            reax_feature)

        # Get the bond change part of the reaction feature.
        bond_changes_reax = self.get_bond_changes_in_reaction(reax_feature)
        ...
        # Update clusters_to_follow, atom_features_in_clusters_to_follow
            ...
            if clusters_in_reactions[i].shape[0] == 1:
                reaction_to_clusters_to_follow \
                    = self.update_one_atom_in_cluster(
                        reaction_to_clusters_to_follow, atom_features,
    ...
    def get_bond_changes_in_reaction(self, reax_feature):
        num_atoms_reax = reax_feature[0]
        num_atoms_features_reax \
                            = 1 + num_atoms_reax * self.num_atom_features
        bond_changes_reax = np.zeros([(reax_feature.shape[0] \
                          - num_atoms_features_reax)//3, 3], dtype=int)
        for i in range(bond_changes_reax.shape[0]):
            bond_changes_reax[i, :] = reax_feature[ \
                num_atoms_features_reax + 3 * i \
                : num_atoms_features_reax + 3 * (i + 1)]
    ...
    def update_one_atom_in_cluster(self, reaction_to_clusters_to_follow, \
                                   atom_features, clusters_in_reactions):
        found = False
        for j in self.clusters_to_follow.keys():
            if np.array_equal( \
                        atom_features[clusters_in_reactions][0],
    ...
    def update_several_atoms_in_cluster(self, reaction_to_clusters_to_follow, 
        ...
        cluster = np.zeros([self.num_atom_features \
                                       * num_atoms_cluster \
                                       + num_atoms_cluster \
        ...
        cluster[ \
            1:self.num_atom_features * num_atoms_cluster + 1] = \
                atom_features[np.sort(clusters_in_reactions), \
                    :].reshape(-1)
        for j in range(num_atoms_cluster - 1):
            for k in range(j + 1, num_atoms_cluster):
                if G.has_edge(clusters_in_reactions[j], 
                    ...
                    idx = self.num_atom_features * num_atoms_cluster\
...
#        self.atoms.update_all_atom_features(bond_matrix, bond_change, self.all_atom_features)  # Update after so we count the reactants before the reactions happen
...
#            pdb.set_trace()
#            for reax in \
#                range(len(self.atom_features_in_clusters_to_follow)):
#                if ((self.all_atom_features[atom] \
#                    == self.atom_features_in_clusters_to_follow[reax])\
#                       .all(-1)).any(-1):
...
#            for reax in range(len(self.atom_features_in_reactions)):
#                if ((self.all_atom_features[atom] == self.atom_features_in_reactions[reax]).all(-1)).any(-1):
...
#                    features = np.vstack([ self.clusters_to_follow[reax][1+ self.num_atom_features*i: 1 + self.num_atom_features*(i+1)]for i in range(num_of_atom)])
...
#                        new_clusters_to_follow = self.search_clusters_to_follow(np.array([]), dict_of_bonds, -1, atom_position, atom, features, bond_matrix)
...
#            if (self.all_atom_features[atom] == list[1 + i*self.num_atom_features: 1 + (i+1)*self.num_atom_features]).all():
...
#            (num_to_idx, self.clusters_to_follow[reax][1 + num_of_atom * self.num_atom_features:].reshape(-1, 1)))
...
#
#    def search_clusters_to_follow(self, list_of_parents_visited, dict_of_bonds, father_idx, atom_number, atom_idx, features, bond_matrix):
#        child = {}
...
#                    if ba !=father_idx and (features[i] == self.all_atom_features[ba]).all():
#                        child[i].append(ba)
#                if child[i] == []:
#                    return np.array([])
#        if len(child) == 0:
...
#            for j in child[i]:
#                subres = self.search_clusters_to_follow(np.append(list_of_parents_visited, atom_number), dict_of_bonds, atom_idx, i, j, features, bond_matrix)
#                if subres.shape[0] != 0:
...
#                                  0] * self.atoms.num_atom_features - 1) // 3
#        for i in range(num_of_bond_change):
#            reactant_1 = atoms_involved[self.reaction_dict[reaction_to_happen][
#                self.reaction_dict[reaction_to_happen][0] * self.atoms.num_atom_features + 1 + i * 3]]
#            reactant_2 = atoms_involved[self.reaction_dict[reaction_to_happen][
#                self.reaction_dict[reaction_to_happen][0] * self.atoms.num_atom_features + 1 + i * 3 + 1]]
#            change = self.reaction_dict[reaction_to_happen][
#                self.reaction_dict[reaction_to_happen][0] * self.atoms.num_atom_features + 1 + i * 3 + 2]
#            if i == 0:
