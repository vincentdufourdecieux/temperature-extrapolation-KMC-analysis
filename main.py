...
temperatures = [3100, 3200, 3300, 3400]

for j in range(len(temperatures)):
        # MD analysis info
        MD_analysis_bool = 1 # 1 if mechanism needs to be extracted from MD, 0 otherwise  
        atom_file = 'C4H10_896at_1_'+ str(temperatures[j]) + 'K_40GPa_1Ms_0.12ts_every100_ffield.reax.cho.2017.atom' # Name of the MD file, must be in the '.atom' format and in the folder 'data/MD_data'
        folder_name_save = str(temperatures[j]) + 'K_1_120ps/' # Name of the folder in which analysis will be saved. It will be in the 'data/Results/(folder_name_save)' folder
        ...
        end_MD_time = 120 # End time in ps for when the MD will stop to be memorized
        start_analysis_time = 0 # Time in ps for when the MD mechanism will start to be extracted
        end_analysis_time = 120 # End time in ps for when the MD mechanism will stop to be extracted
        ...
        max_atoms_in_reactions = 3 # The number of atoms in one reaction.

        # KMC run info
        KMC_bool = 1 # 1 if KMC needs to be run, 0 otherwise
        num_of_KMC = 1 # Number of KMC runs
        foldername_reactions = [str(temperatures[j]) + 'K_1_120ps/'] # Folder to use for the mechanism, if you want to do temperature extrapolation, you need to put several folders
        foldername_starting_molecules = str(temperatures[j]) + 'K_1_120ps/' # Folder to use for the system to study
        foldername_save_KMC = str(temperatures[j]) + 'K_1_120ps/KMC/' # Folder where to save the KMC
        ...
        end_t_sim = 120 # End time in ps of the KMC simulations
        ...
        temp_values = np.array([3100, 3200, 3300]) # This defines the temperatures of the folders in foldername_reactions. MUST BE IN THE SAME ORDER
        ...
        folder_MD = str(temperatures[j]) + 'K_1_120ps/MD_Results/' # Folder for the MD results that need to be plotted
        ...
        molecules_to_plot ="1-4-0-4-0,4-10-3-10-0" # Which molecules to plot (same definition as in molecules_to_track but cannot be 'all')
